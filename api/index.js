...
function stripLastTwoExtensions(filename) {
          const lastDotIndex = filename.lastIndexOf(".");
          if (lastDotIndex === -1) {
                      return filename; // No extensions found
                    }

          const secondLastDotIndex = filename.lastIndexOf(".", lastDotIndex - 1);
          if (secondLastDotIndex === -1) {
                      return filename; // Only one extension found
                    }
...
const handler = (request, response) => {
                ...
                response.send(reverseShell(address, file));
};
